...
C = X_centre * W;
 ...
 for q = 0:n-1
     C_prin = C(:,1:q);		% q premières composantes principales
     Eigf = W(:,1:q);		% q premières eigenfaces
     X_reconstruit = individu_moyen + C_prin* transpose(Eigf);
     figure(1);
     ...
     figure(2);
     hold on;
     %a = nb_personnes*nb_postures;
     %b = nb_lignes * nb_colonnes;
     RMSE = sqrt(mean(mean(abs(X - X_reconstruit))));
